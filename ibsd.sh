...
download_and_extract_freebsd(){
	 chflags -R noschg ${ibsd_root}
	 rm -rf ${ibsd_root}
	 mkdir -p ${boot_dir} && chown root:wheel ${ibsd_root} && chown root:wheel ${boot_dir} 
	 mkdir -p ${boot_dir}/kernel ${boot_dir}/modules && chown -R root:wheel ${boot_dir}
	 
	 log "Downloading base and kernel ..."
	 fetch  -o - "https://download.freebsd.org/ftp/releases/${arch}/${version}/base.txz" > "${ibsd_root}/base.txz"
	 fetch  -o - "https://download.freebsd.org/ftp/releases/${arch}/${version}/kernel.txz" > "${ibsd_root}/kernel.txz"
	 log "Extracting base and kernel ..."
	 cat "${ibsd_root}/base.txz" | tar --unlink -xpzf - -C  "${dest_dir}"
	 cat "${ibsd_root}/kernel.txz" | tar --unlink -xpzf - -C "${root_dir}"
}
...
copy_cdboot_and_efi_loader(){
	...
	mkdir -p ${ibsd_root}/cdboot
	cp -f ${dest_dir}/boot/cdboot ${ibsd_root}/cdboot/
	cp -f ${dest_dir}/boot/loader_4th.efi ${ibsd_root}/cdboot/
	cp -f ${dest_dir}/boot/loader_lua.efi ${ibsd_root}/cdboot/
}
...
chmod 0555 ${dest_dir}/etc/rc.d/packages
	...
	cat > ${dest_dir}/etc/rc.d/ibsd<<"EOF"
#!/bin/sh
# $Id$
...
run_rc_command "$1"	
EOF
chmod 0555 ${dest_dir}/etc/rc.d/ibsd
...
create_boot_environment(){
	...
	mkdir -p ${ibsd_root}/disk/boot/kernel && chown  root:wheel ${ibsd_root}/disk
	local exclude_file=$(mktemp)
	echo -e "kernel.debug\n*.symbols\n*.ko" > "$exclude_file"
	tar -c -X ${exclude_file} -C ${boot_dir}/kernel -f - . | tar -xv -C ${ibsd_root}/disk/boot/kernel -f -
	rm "$exclude_file"
	
	cp -f -rp ${dest_dir}/boot.config ${ibsd_root}/disk
	local files='defaults device.hints loader_lua lua'
	for _file in $(echo "$files"); do
		cp -f -rp ${dest_dir}/boot/${_file} ${ibsd_root}/disk/boot
	done
	
	mv -f ${ibsd_root}/disk/boot/loader_lua ${ibsd_root}/disk/boot/loader
	rm -rf ${ibsd_root}/disk/boot/kernel/*.ko  ${ibsd_root}/disk/boot/kernel/*.symbols
	
	log 'Copying acpi ...'
	find ${boot_dir}/kernel -name 'acpi*.ko' -exec install -m 0555 {} ${ibsd_root}/disk/boot/kernel \;
	
	log 'Copying BOOTMODULES ...'
	local BOOTMODULES='acpi ahci'
	for _file in $BOOTMODULES; do
		[ ! -f ${boot_dir}/kernel/${_file}.ko ] || \
			install -m 0555 ${boot_dir}/kernel/${_file}.ko ${ibsd_root}/disk/boot/kernel
	done
	...
	mkdir -p ${ibsd_root}/boot
	cp -f -p ${dest_dir}/boot/pmbr ${dest_dir}/boot/gptboot ${ibsd_root}/boot
}

create_efi_boot_image(){
	log 'Creating EFI boot image ...'
	mkdir -p ${ibsd_root}/efiroot/EFI/BOOT
	cp -f ${ibsd_root}/cdboot/loader_lua.efi ${ibsd_root}/efiroot/EFI/BOOT/BOOTX64.efi
	makefs -t msdos -s 2048k -o fat_type=12,sectors_per_cluster=1 ${ibsd_root}/cdboot/efiboot.img ${ibsd_root}/efiroot
}
...
install_pkgng(){
	...
	PKG_ABI="${PKG_ABI}" \
	PKG_CACHEDIR="${ibsd_root}/pkgcache" \
...
create_and_compress_ibsd(){
	log 'Creating and compressing ibsd ...'
	mkdir ${ibsd_root}/mnt
	local max_size='256m'
	local free_inodes='10%'
	local free_blocks='10%'
	makefs  -t ffs -m ${max_size} -f ${free_inodes} -b ${free_blocks} ${ibsd_root}/disk/ibsd ${root_dir} > /dev/null
	rm -rf ${ibsd_root}/mnt
	gzip -9 -f ${ibsd_root}/disk/ibsd
	gzip -9 -f ${ibsd_root}/disk/boot/kernel/kernel
	
	if [ -f "${dest_dir}/boot/loader.conf" ]; then
		install -m 0644 ${dest_dir}/boot/loader.conf ${ibsd_root}/disk/boot/loader.conf
	fi
}

create_img_ibsd(){
	log 'Creating image ibsd ...'
	#default gptpart or bsdpart
	local fssize=0
	local fslable='auto'
	local fsimg='ibsd.img'
	local fsproto="${ibsd_root}/disk"
	local tmpimg=$(mktemp -t ${fsimg})
	local efiimg="${ibsd_root}/cdboot/efiboot.img"
...
create_iso_ibsd(){
	log 'Creating ISO ibsd ...'
	makefs -t cd9660 -o rockridge,label=ibsd \
	-o bootimage=i386\;${ibsd_root}/cdboot/cdboot,no-emul-boot \
	-o bootimage=i386\;${ibsd_root}/cdboot/efiboot.img,no-emul-boot,platformid=efi \
	ibsd.iso ${ibsd_root}/disk
	
	#makefs -t cd9660 -o rockridge,label=ibsd \
	#-o bootimage=i386\;${ibsd_root}/cdboot/cdboot,no-emul-boot \
	#ibsd-bios.iso ${ibsd_root}/disk
}
...
# rm /usr/local/www/nginx/ibsd* ;cp ibsd* /usr/local/www/nginx;ls /usr/local/www/nginx
# http://192.168.1.1/ibsd.img
# rm -rf /boot/images/* ; curl -o /boot/images/ibsd.iso http://192.168.1.1/ibsd.iso;update-grub
...
#	insmod part_gpt
#	set root=(hd0,gpt2)
#	set isofile="/@/boot/images/ibsd.iso"
...
#	echo "ahci.ko loaded"
#	kfreebsd_module (loop)/ibsd.gz type=mfs_root
#	set kFreeBSD.vfs.root.mountfrom="ufs:/dev/md0"
#	set kFreeBSD.mfs_type="mfs_root"
#	set kFreeBSD.mfs_name="/ibsd"
